...
**Version:** 3.0 (Ansible-based, multi-platform, env-folder pattern)  
**Status:** Proxmox working (pre-refactor test pending), OVH planned  
**Date:** 2026-02-24
...
| **Gitignored env folders** | Real env folders (`proxmox-*`, `ovh-*`) are gitignored. Only `*-example/` are committed. Sync real folders to a secure cloud backup. |
...
| **Split playbooks per platform** | `deploy-proxmox.yml`, `deploy-ovh.yml`, `destroy-proxmox.yml`, `destroy-ovh.yml`. No platform checks inside playbooks — clean linear flow. Playbook validates `platform` field matches and fails early if wrong. |
| **Cilium via Helm** | Post-bootstrap `helm install` is simpler than embedding 1600+ lines of YAML in Talos config. |
| **No `machine.network.hostname`** | dnsmasq handles it (Proxmox) or Talos auto-generates (OVH). |
...
├── ansible/
│   ├── ansible.cfg
│   ├── deploy-proxmox.yml         # Proxmox deploy (reads env folder)
│   ├── deploy-ovh.yml             # OVH deploy
│   ├── destroy-proxmox.yml        # Proxmox destroy
│   ├── destroy-ovh.yml            # OVH destroy
│   └── roles/
...
│   │   └── config.yml
│   └── ovh-example/               # Committed — copy for new OVH env
│       └── config.yml
│
│   # Real envs are GITIGNORED (contain secrets + generated files):
│   # environments/proxmox-homelab/
│   # environments/ovh-prod/
│
...
```yaml
platform: proxmox           # Controls which roles run
...
### OVH VPS (`environments/ovh-example/config.yml`)

```yaml
platform: ovh               # Controls which roles run

# Talos + cluster (no VIP on OVH)
talos_version: v1.12.4
talos_schematic_id: e187c9b...
cluster_name: fluodata
...
```
deploy-proxmox.yml -e env=../environments/proxmox-homelab
    │
    ├── Load config.yml → validate platform=proxmox
    ├── Add Proxmox SSH host to dynamic inventory
    │
    ├── proxmox_vms (on Proxmox host via API)
    │   └── Download ISO, create VMs, start VMs
    │
    ├── talos_bootstrap (talosctl on Proxmox host, fetch results to env folder)
    │   └── Secrets → patches → configs → apply → bootstrap → kubeconfig
    │
    ├── proxmox_post_install (on Proxmox host)
    │   └── Switch boot order to disk, detach ISO
    │
    └── cilium (helm on Proxmox host)
        └── Install Cilium, wait for nodes Ready
```

### OVH Flow

```
deploy-ovh.yml -e env=../environments/ovh-prod
    │
    ├── Load config.yml → validate platform=ovh
    │
    ├── talos_install (SSH to VPS in rescue mode)
    │   └── dd Talos image → reboot
    │
    ├── talos_bootstrap (talosctl locally, files in env folder)
    │   └── Secrets → patches → configs → apply → bootstrap → kubeconfig
    │
    └── cilium (helm locally)
        └── Install Cilium, wait for nodes Ready
...
### Env folders
- Gitignored (`environments/proxmox-*/`, `environments/ovh-*/` except `*-example/`)
...
### OVH network
- Talos has NO host firewall — OVH network firewall is mandatory
...
7. **OVH VPS rescue mode** — only viable install path (custom ISO not supported)
...
- **OVH firewall automation** — via OVH API
